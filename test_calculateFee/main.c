...
int main(int argc, char **argv)
{
	...
	int inputList[argc - 2];  // 개별 수수료를 담을 배열
	int listSize = sizeof(inputList) / sizeof(int) - 1; // 배열의 크기 
	int i;
...
{
	int length = listSize;
	int tempList[listSize]; // 임시배열
	int step = 0; // 단계
	int i, rtn;
	int tempListIndex = 0; //tempList 배열의 index

	for (i = 0; i < listSize; i++) tempList[i] = 0;  // tempList 배열 초기화

	// inputList 배열의 총합이 결과 값과 같은지 체크
...
{
	int length = listSize;
	int finalList[listSize]; // 결과값이 담길 배열
	int finalListIndex = 0; // finalList 배열의 index
	int total = 0; // finalList 배열의 합계
	int isChecked = 0; // bool변수, 결과값과 같으면 참
	int i, j;
	
	for (i = 0; i < listSize; i++) finalList[i] = 0; // finalList 배열 초기화
