...
long sem_key[10] = {'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'}; 
long shm_key[10] = {'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T'};
...
int main(int argc, char **argv)
{
	...
	shared_status = ini_readinteger(initFile, "SHARED_INFO", "SHARED_STATUS", 0);

	Sig_Debug_Set(SIG_DEBUG);

	// 세마포어, 공유메모리 초기화
	r = init_ipc(sem_key, shm_key);
	if (r < 0)
	{
		dbg(9, "ERR: ipc initialize error(%d) <%s>", r, __fn);
		return -1;
	}
	...
	// 2. tcp 
	r = server_daemon(port, shared_status);
	if (r < 0)
	{
