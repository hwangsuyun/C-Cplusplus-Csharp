...
int main()
{
	...
	BYTE pbUserKey[16] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 		// User secret key
                        0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10};
	//BYTE pbData[500]    = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 		// input plaintext to be encrypted
    //                    0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};
	//BYTE pbData[16]    = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 		// input plaintext to be encrypted
    //                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	int i;
	unsigned char pbData[500];
	memset(pbData, 0x00, sizeof(pbData));
	//memcpy(pbData, "1111222233334444", sizeof(pbData));
	memcpy(pbData, "62103950****5952", sizeof(pbData));
...
printf("strlen(pbData) = %d\n", strlen(pbData));
	...
	memcpy(inputData, pbData, sizeof(inputData));

	base64e(inputData,outputData,strlen(inputData));

	for(i=0;i<strlen(outputData);i++)
	{
	    printf("outputData[%d]=>[%c]\n",i,outputData[i]);
	}
	
	base64d(outputData, resultData, &length);
	for (i=0;i<16;i++)	
