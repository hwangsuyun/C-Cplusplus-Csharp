...
/*
	clser_qid = msgget( CLSER_QKEY, IPC_CREAT | QPERM);
	if (clser_qid < 0){
		LOGDEBUG(0, "[%s] msgget clser_qid failed", func);
		return -1;
	}
	
	req_qid = msgget( REQ_QKEY, IPC_CREAT | QPERM);
	if (req_qid < 0){
		LOGDEBUG(0, "[%s] msgget req_qid failed", func);
		return -1;
	}
	
	van_qid = msgget( VAN_QKEY, IPC_CREAT | QPERM);
	if (van_qid < 0){
		LOGDEBUG(0, "[%s] msgget van_qid failed", func);
		return -1;
	}
	
	rep_qid = msgget( REP_QKEY, IPC_CREAT | QPERM);
	if (rep_qid < 0){
		LOGDEBUG(0, "[%s] msgget rep_qid failed", func);
		return -1;
	}
	
	rev_qid = msgget( REV_QKEY, IPC_CREAT | QPERM);
	if (rev_qid < 0){
		LOGDEBUG(0, "[%s] msgget rev_qid failed", func);
		return -1;
	}
	
	tlflog_qid = msgget( TLFLOG_QKEY, IPC_CREAT | QPERM);
	if (tlflog_qid < 0){
		LOGDEBUG(0, "[%s] msgget tlflog_qid failed", func);
		return -1;
	}
	
	tlfbroker_qid = msgget( TLFBROKER_QKEY, IPC_CREAT | QPERM);
	if (tlfbroker_qid < 0){
		LOGDEBUG(0, "[%s] msgget tlfbroker_qid failed", func);
		return -1;
	}
	
	stlf_qid = msgget( STLF_QKEY, IPC_CREAT | QPERM);
	if (stlf_qid < 0){
		LOGDEBUG(0, "[%s] msgget stlf_qid failed", func);
		return -1;
	}
*/
	s.val = 1;
...
/*	
	vangrp_shmid = shmget( VANGRP_SHMKEY, sizeof(VANGRP) , QPERM);
	if ( vangrp_shmid < 0){
		vangrp_shmid = shmget( VANGRP_SHMKEY, sizeof( VANGRP ), IPC_CREAT | QPERM);
		if ( vangrp_shmid < 0){
			LOGDEBUG(0, "[%s] vangrp_shmid get failed", func);
			return -1;
		}

		vangrp = (VANGRP*) shmat (vangrp_shmid, (void*)0, 0);
		if (vangrp == (VANGRP*) -1){
			LOGDEBUG(0, "[%s] vangrp shmat failed", func);
			return -1;
		}
...
*/
	...
	/*		PIPE		CREATE			: START			*/
/*	
	strcpy( fifo_name , "./.trs_q" );
	ret		= access( fifo_name , F_OK );
	if( ret < 0 ) {
		ret		= mknod( fifo_name , S_IFIFO | 0666 , 0 );
		if( ret < 0 ) {
			LOGDEBUG( 0 , "[%s] mknod fail errno[%s %d]" , func , strerror(errno) , errno );
			return		-1;
		}

		LOGDEBUG( 3 , "[%s] fifo file create [%s]" , func , fifo_name );
	}
	
	strcpy( fifo_name , "./.trs_p" );
	ret		= access( fifo_name , F_OK );
	if( ret < 0 ) {
		ret		= mknod( fifo_name , S_IFIFO | 0666 , 0 );
		if( ret < 0 ) {
...
/*	
	rc = shmctl(vangrp_shmid, IPC_RMID, (struct shmid_ds*) 0);
	if ( rc < 0){
		LOGDEBUG(0, "[%s] shmctl failed vangrp_shmid", func);
			result = -1;
	}
...
/*
	rc = msgctl(clser_qid, IPC_RMID, (struct msqid_ds*) 0);
	if ( rc < 0){
		LOGDEBUG(0, "[%s] msgctl failed clser_qid", func);
			result = -1;
	}

	rc = msgctl(req_qid, IPC_RMID, (struct msqid_ds*) 0);
	if ( rc < 0){
		LOGDEBUG(0, "[%s] msgctl failed req_qid", func);
			result = -1;
	}

	rc = msgctl(van_qid, IPC_RMID, (struct msqid_ds*) 0);
	if ( rc < 0){
		LOGDEBUG(0, "[%s] msgctl failed van_qid", func);
			result = -1;
	}

	rc = msgctl(rep_qid, IPC_RMID, (struct msqid_ds*) 0);
	if ( rc < 0){
		LOGDEBUG(0, "[%s] msgctl failed rep_qid", func);
			result = -1;
	}

	rc = msgctl(rev_qid, IPC_RMID, (struct msqid_ds*) 0);
	if ( rc < 0){
		LOGDEBUG(0, "[%s] msgctl failed rev_qid", func);
			result = -1;
	}

	rc = msgctl(tlfbroker_qid, IPC_RMID, (struct msqid_ds*) 0);
	if ( rc < 0){
		LOGDEBUG(0, "[%s] msgctl failed tlfbroker_qid", func);
			result = -1;
	}

	rc = msgctl(tlflog_qid, IPC_RMID, (struct msqid_ds*) 0);
	if ( rc < 0){
		LOGDEBUG(0, "[%s] msgctl failed tlflog_qid", func);
			result = -1;
	}

	rc = msgctl(stlf_qid, IPC_RMID, (struct msqid_ds*) 0);
	if ( rc < 0){
		LOGDEBUG(0, "[%s] msgctl failed stlf_qid", func);
			result = -1;
	}
*/		
...
LOGDEBUG(1, "[%s]", func);
		...
		if( rc == 0){
			LOGDEBUG(3,"[%s] select timeout", func);
			ProcessTimer();
