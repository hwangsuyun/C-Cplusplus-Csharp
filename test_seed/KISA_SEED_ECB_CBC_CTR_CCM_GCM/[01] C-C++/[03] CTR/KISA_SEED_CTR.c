...
#define BLOCK_XOR_SEED( OUT_VALUE, IN_VALUE1, IN_VALUE2 ) {	\
	...
	case NONCE_OR:
		{
			...
			pbszNonce[8] = pbszIV[8] | pbszCounter[8];
			pbszNonce[9] = pbszIV[9] | pbszCounter[9];
			pbszNonce[10] = pbszIV[10] | pbszCounter[10];
			pbszNonce[11] = pbszIV[11] | pbszCounter[11];
			pbszNonce[12] = pbszIV[12] | pbszCounter[12];
			pbszNonce[13] = pbszIV[13] | pbszCounter[13];
			pbszNonce[14] = pbszIV[14] | pbszCounter[14];
			pbszNonce[15] = pbszIV[15] | pbszCounter[15];
		}
		break;
	case NONCE_AND:
		{
...
void KISA_SEED_Block_forCTR( unsigned int *in, unsigned int *out, KISA_SEED_KEY *ks )
{
...
int SEED_CTR_init( OUT KISA_SEED_INFO *pInfo, IN KISA_ENC_DEC enc, IN BYTE *pbszUserKey, IN BYTE *pbszCounter )
{
	...
	pInfo->encrypt = enc;
	memcpy( pInfo->ctr, pbszCounter, 16 );
...
int SEED_CTR_Process( OUT KISA_SEED_INFO *pInfo, IN DWORD *in, IN int inLen, OUT DWORD *out, OUT int *outLen )
{
	...
	{
		KISA_SEED_Block_forCTR( pdwCounter, out, &pInfo->seed_key );
		BLOCK_XOR_SEED( out, in, out );

		UpdateCounter_for_SEED( pbszCounter, 1, (BLOCK_SIZE_SEED-1) );
...
int SEED_CTR_Encrypt( IN BYTE *pbszUserKey, IN BYTE *pbszCTR, IN BYTE *pbszInputText, IN int nInputTextLen, OUT BYTE *pbszOutputText )
{
	int nOutLeng = 0;
	int nEncRmainLeng = 0;
	KISA_SEED_INFO info;
	...
	int nInputTextPadding = (BLOCK_SIZE_SEED - (nInputTextLen % BLOCK_SIZE_SEED)) % BLOCK_SIZE_SEED;
	newpbszInputText = (unsigned char *) malloc(sizeof(unsigned char) * (nInputTextLen + nInputTextPadding));
	memcpy(newpbszInputText, pbszInputText, nInputTextLen);

	SEED_CTR_init( &info, KISA_ENCRYPT, pbszUserKey, pbszCTR );

	outlen = ((nInputTextLen + nInputTextPadding)/16) * 4;

	outbuf = malloc(sizeof(unsigned int) * outlen);
	data = chartoint32_for_SEED_CTR(newpbszInputText, nInputTextLen );

	SEED_CTR_Process( &info, data, nInputTextLen, outbuf, &nOutLeng );
	SEED_CTR_Close( &info, outbuf+(nOutLeng)/4, &nEncRmainLeng);
...
int SEED_CTR_Decrypt( IN BYTE *pbszUserKey, IN BYTE *pbszCTR, IN BYTE *pbszInputText, IN int nInputTextLen, OUT BYTE *pbszOutputText )
{
	int nOutLeng = 0;
	int nEncRmainLeng = 0;
	KISA_SEED_INFO info;
...
void main()
{
	...
	printf ("\nCounter	: ");
	for (i=0;i<16;i++)	{printf("%02X ",pbszCounter[i]);}
...
void main()
{
	...
	printf ("\nPlaintext(%d)  : ", nInputTextLen);
	for (i=0;i<nInputTextLen;i++)	{printf("%02X ",InputBytes[i]);}
	...
	{
		SEED_CTR_init( &info, KISA_ENCRYPT, pbszUserKey, pbszCounter );
		...
		for (i = 0; i < message_length - process_blockLeng; )
		{
			...
			data = chartoint32_for_SEED_CTR(pbszInputText, process_blockLeng);
			SEED_CTR_Process( &info, data, process_blockLeng, outbuf, &nOutLeng );
		...
		data = chartoint32_for_SEED_CTR(pbszInputText, remainleng);
		SEED_CTR_Process( &info, data, remainleng, outbuf, &nOutLeng );
		SEED_CTR_Close( &info, outbuf+(nOutLeng)/4, &nEncRmainLeng );
		cdata = int32tochar_for_SEED_CTR(outbuf, nOutLeng - nEncRmainLeng);
	...
	else
	{
		...
		for (i = 0; i < message_length - process_blockLeng; )
		{
			...
			data = chartoint32_for_SEED_CTR(pbszOutputText, process_blockLeng);
			SEED_CTR_Process( &info, data, process_blockLeng, outbuf, &nOutLeng );
			cdata = int32tochar_for_SEED_CTR( outbuf, nOutLeng);
		...
		data = chartoint32_for_SEED_CTR(pbszOutputText, remainleng);
		SEED_CTR_Process( &info, data, remainleng, outbuf, &nOutLeng );
		SEED_CTR_Close( &info, outbuf+(nOutLeng)/4, &nEncRmainLeng );
		cdata = int32tochar_for_SEED_CTR(outbuf, nOutLeng - nEncRmainLeng);
